 ...
 */
import { CognitoAuthClient } from './auth/authClient';
import { checkClaimOwnership } from './claims/claimOwnership';
import { buildClaimStatusResponse } from './claims/claimStatusEndpoint';
import { checkSessionTimeout } from './auth/sessionTimeout';
import { checkLockout, recordFailedAttempt, DEFAULT_LOCKOUT_CONFIG } from './auth/lockoutTracking';
import { DEFAULT_SYSTEM_CONFIG } from '@claims/shared';
import { CognitoIdentityProviderClient } from '@aws-sdk/client-cognito-identity-provider';

const cognitoClient = new CognitoIdentityProviderClient({});
const authClient = new CognitoAuthClient(cognitoClient);
...
export async function handler(event: any): Promise<any> {
  ...
  try {
    ...
    if (path.match(/\/claims\/[^/]+\/?$/) && method === 'GET') {
      ...
      const statusResponse = buildClaimStatusResponse(claimId, 'Intake', [
    ...
    if (path.includes('/disputes') && method === 'POST') {
      ...
      if (reason.trim().length > DEFAULT_SYSTEM_CONFIG.maxDisputeReasonLength) {
        return response(400, {
          message: `Dispute reason exceeds maximum length of ${DEFAULT_SYSTEM_CONFIG.maxDisputeReasonLength} characters.`,
        });
      }
...
function handleCognitoTrigger(event: any): any {
...
function extractClaimId(path: string): string {
